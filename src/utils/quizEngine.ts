...
const QUESTION_WEIGHT_FALLBACKS: Record<
  DimensionPair,
  Partial<Record<QuestionArchetypeWeightId, number>>
> = {
...
export const TRAIT_CONFIG: Record<DimensionPair, TraitConfig> = {
  E_I: {
    label: '能量',
...
function buildAnswerProfile(input: {
  ...
  ;(Object.keys(DIMENSION_LETTERS) as DimensionPair[]).forEach((pair) => {
...
function isAnsweredValue(value: number | null | undefined): value is number {
  return typeof value === 'number' && value >= -3 && value <= 3
}

function normalizeDimensionScore(rawScore: number, directionalMax: { positive: number; negative: number }) {
  if (rawScore >= 0) {
...
function scoreMbti(matchCode: string, scores: Record<DimensionPair, DimensionScore>) {
...
function scoreFlexibleMbti(character: CharacterMatch, scores: Record<DimensionPair, DimensionScore>) {
...
function scoreVector(userVector: UserVector, characterVector: CharacterMatch['vector']) {
...
function scoreCharacterSpecific(
  ...
  const axisScore =
    !uniqueAxes || !Object.keys(uniqueAxes).length
...
function scoreUniqueAxes(userVector: UserVector, uniqueAxes: Partial<Record<DimensionId, number>>) {
  let weightedScore = 0
  let weightTotal = 0
...
function cosineSimilarity(left: UserVector, right: CharacterMatch['vector']) {
  let dot = 0
  let leftMagnitude = 0
...
export function buildScoresFromMbtiCode(mbtiCode: string, percentages?: Partial<Record<DimensionPair, number>>) {
  ...
  const defaults: Record<DimensionPair, number> = {
    E_I: 74,
    S_N: 70,
    T_F: 68,
    J_P: 72,
  ...
  return (['E_I', 'S_N', 'T_F', 'J_P'] as DimensionPair[]).reduce(
    (acc, pair, index) => {
